...
static void
sort_chunks (char *subprocess, int exit_status)
{
  ...
  for (i = 0; i < CHUNK_CNT; i++) 
    {
      ...
      CHECK ((children[i] = exec (subprocess, (char *[]) { subprocess, fn, (char *) 0 })) != -1, "exec \"%s %s\"", subprocess, fn);
      quiet = false;
    }
...
void
parallel_merge (char *child_name, int exit_status)
{
  init ();
